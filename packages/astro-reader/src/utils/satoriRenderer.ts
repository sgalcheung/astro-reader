...
async function getFontBuffer() {
	...
	try {
		...
		console.warn(
			`[astro-reader] Fetching font from ${fontUrl} failed. Falling back to local fs.readFile.`,
		);
		...
		try {
			...
			if (!localFontBuffer) {
				throw new Error(
					"No .ttf or .otf font files were found in the public/fonts or public directory.",
				);
			}
